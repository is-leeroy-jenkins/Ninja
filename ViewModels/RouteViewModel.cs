...
using static Ninja.Models.RouteModel;
...
namespace Ninja.ViewModels
{
    ...
    internal class RouteViewModel : MainWindowBase
    {
        ...
        public RouteModel.NetInterfaceInfo[] GetLocalNetworkInterface()
        {
            NetworkInterface[] interfaces = NetworkInterface.GetAllNetworkInterfaces();
            int len = interfaces.Length;
            RouteModel.NetInterfaceInfo[] info = new RouteModel.NetInterfaceInfo[len];
            //string[] name = new string[len];
            for (int i = 0; i < len; i++)
            {
                NetworkInterface ni = interfaces[i];
                info[i] = new RouteModel.NetInterfaceInfo();
